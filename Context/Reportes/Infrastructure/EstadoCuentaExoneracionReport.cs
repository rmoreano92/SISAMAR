...
{
    public class EstadoCuentaExoneracionReport : IExcelReportGenerator
    {
        public void Generar(IXLWorksheet ws, DataSet dataSet, List<DatosCabeceraReporte> cabeceras)
        {
            ...
            ws.Cell(4, 3).Value = $"{cabeceras[0].CabeceraPrincipal.Direccion}";
            ws.Cell(5, 3).Value = $"{cabeceras[0].CabeceraPrincipal.Ocupación}";
            // ws.Cell(6, 3).Value = $"{cabeceras[0].CabeceraPrincipal.}";
            ws.Cell(3, 8).Value = $"{cabeceras[0].CabeceraPrincipal.NumeroHistoriaClinica}";
            ws.Cell(4, 8).Value = $"{cabeceras[0].CabeceraPrincipal.IdCuentaAtencion}";
            ws.Cell(5, 8).Value = $"{cabeceras[0].CabeceraPrincipal.FechaEgreso}";

            var rango1 = ws.Range("B7:I8");
            rango1.Value = $"{cabeceras[0].CabeceraPrincipal.RiesgoSocial}";
            rango1.Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Center;
            rango1.Style.Font.Bold = true;

            var rango = ws.Range("B11:I11");
            rango.Merge(); // Une las celdas
            rango.Value = $"SERVICIO EGRESO: {cabeceras[0].CabeceraPrincipal.ServicioEgreso} ({cabeceras[0].CabeceraPrincipal.TipoServicio})";
            ...
            // ws.Cell(filaActual, 2).Value = "SUBTOTAL";
            ws.Cell(filaActual, 5).Value = importeExonera.ToString("N2");
            // ws.Cell(filaActual, 4).Value = "";
            ...
            foreach (DataRow row in dataTableServicios.Rows)
            {
                ...
                {
                    ws.Cell(filaActual, 2).Value = index;
                    ws.Cell(filaActual, 3).Value = puntoCargaActual;
                    // ws.Cell(filaActual, 2).Value = "SUBTOTAL";
                    ws.Cell(filaActual, 5).Value = importeExonera.ToString("N2");
                    // ws.Cell(filaActual, 4).Value = "";
                    ws.Cell(filaActual, 7).Value = enletras(importeExonera.ToString("N2"));
            ...
            ws.Cell(filaActual + 2, 4).Value = "MONTO LIQUIDACIÓN BRUTO";
            ws.Cell(filaActual + 2, 6).Value = montoLiquidacion.ToString("N2");

            ws.Cell(filaActual + 3, 4).Value = "TOTAL A EXONERAR (DESCUENTO)";
            ws.Cell(filaActual + 3, 6).Value = importeExoneraGeneral.ToString("N2");
            ws.Cell(filaActual + 3, 7).Value = enletras(importeExoneraGeneral.ToString("N2"));

            ws.Cell(filaActual + 4, 4).Value = "TOTAL A PAGAR";
            ws.Cell(filaActual + 4, 6).Value = saldoGeneral.ToString("N2");
            ws.Cell(filaActual + 4, 7).Value = enletras(saldoGeneral.ToString("N2"));
        }
