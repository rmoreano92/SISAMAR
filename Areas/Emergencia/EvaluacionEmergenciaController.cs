...
namespace WebAppMaternidad.Areas.Emergencia
{
    public class EvaluacionEmergenciaController : BaseController
    {
        ...
        public EvaluacionEmergenciaController(IWebHostEnvironment env)
        {
        ...
        [HttpPost]
        public async Task<ActionResult> SeleccionarPapeletasDetalle(int idAtencion, int idServicio)
        {
            if (HttpContext.User.Identity.IsAuthenticated == false)
            {
                return Json(new { session = false });
            ...
            int idUsuario;
            idUsuario = int.Parse(HttpContext.Session.GetString("idusu"));
            ds = await daoEvaluacion.SeleccionarPapeletasDetalle(idAtencion, idServicio, idUsuario);
            //hoja = await GenerarHojaRefCon(objRef.IdCuentaAtencion, Convert.ToInt32(ds.Tables[0].Rows[0]["IdReferencia"].ToString()), 1, "RF");
        ...
        public async Task<Boolean> GenerarHojaEvaluacion(int idCuenta, int idAtencion, int idEvaluacionDetalle, int idServicio, int eval)
        {
            try
            {
                ...
                stringHtml = null;
                pageHtml = null;
                pageHtml = Url.Action("InformeEvaluacion", "EvaluacionEmergencia", new { area = "Emergencia", idAtencion, idServicio, eval, usuario }, "http");
        ...
        public async Task<ActionResult> InformeEvaluacion(int idAtencion, int idServicio, int eval, string usuario)
        {
            ...
            @ViewBag.RiesgoCaida = DatosEvaluacion.Tables[0].Rows[0]["RiesgoCaida"];
            ...
            @ViewBag.InformarFamiliar = DatosEvaluacion.Tables[0].Rows[0]["InformarFamiliar"];
            @ViewBag.InformacionFamiliar = DatosEvaluacion.Tables[0].Rows[0]["InformacionFamiliar"];

            @ViewBag.RiesgoFuga = DatosEvaluacion.Tables[0].Rows[0]["RiesgoFuga"];
            ...
            if (eval == 1)
            {
                return PartialView("~/Views/Emergencia/Plantillas/InformeEvaluacionEmergencia.cshtml");
            }
            else
            {
                return PartialView("~/Views/Emergencia/Plantillas/InformeReEvaluacionEmergencia.cshtml");
            }
