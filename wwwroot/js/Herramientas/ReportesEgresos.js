let ReportesEgresos = {
    ...
    async Iniciar() {
        await ReportesEgresos.Plugins();
        ReportesEgresos.Eventos();
    },
    ...
    async Plugins() {
        ...
        $('#txtFechaInicioEgresos, #txtFechaFinEgresos').datepicker({ 
            todayHighlight: true,
            autoclose: true,
            orientation: "bottom"
        ...
        $("#txtFechaInicioEgresos, #txtFechaFinEgresos").datepicker('setDate', fechaActual.substring(0, 10));
        ...
        $("#txtFechaInicioEgresos, #txtFechaFinEgresos").mask("Dd/Mm/abcd");
    ...
    Eventos() {
        $('#btnGenerarReporteEgresos').on('click', () => {

            if (isEmpty($('#txtFechaInicioEgresos').val())) {
            ...
            if ($('#txtFechaInicioEgresos').val() > $('#txtFechaInicioEgresos').val()) {
                alerta2('info', '', 'La fecha de inicio debe ser menor o igual a la fecha final.');
                return false
            }

            if (isEmpty($('input[name="rdbTipoReporteEgresos"]:checked').val())) {
            ...
            formData.append('fechaInicio', $("#txtFechaInicioEgresos").val());
            formData.append('fechaFin', $("#txtFechaFinEgresos").val());
            formData.append('idTipoServicio', $("#cboTiposServicioEgresos").val());
            formData.append('idDepartamento', $("#cboDepartamentoEgresos").val());
            formData.append('idEspecialidad', $("#cboEspecialidadEgresos").val());
            formData.append('idServicio', $("#cboServicioEgresos").val());
            formData.append('idFuenteFinanciamiento', $("#cboFuenteFinanciamientoEgresos").val());
            formData.append('considerarCpt', $("#chkConsiderarCPT").is(':checked'));
            formData.append('considerarDx', $("#chkConsiderarDx").is(':checked'));
            formData.append('estancia24h', $("#chkEstancia24h").is(':checked'));


            formData.append('tipoReporte', $('input[name="rdbTipoReporteEgresos"]:checked').val());
            ...
            fetch('/ReportesEgresos/rptListarReporteEgresos?area=Herramientas', {
                method: "POST",
                body: formData
            })
                .then(response => {
        ...
        //$('#btnCerrarHerrReportesEgresos').on('click', async () => { 
        //    ReportesEgresos.LimpiarCampos();
        //    $("#modalReportesEgresos").modal("hide");
        //});
    },
