var IntervencionSanitaria = {
    ...
    Eventos() {
        ...
        $('#btnGuardarVentas').on('click', async function () {
            ...
            if ($('#rdbPreventa').is(':checked')) {
                let ventaDirecta = IntervencionSanitaria.AgregarDatosDePreVenta()
    ...
    async AgregarDatosDeVentaDirecta() {
        ...
        let productosLotesParaDespachoIntervencionSanitaria = []
        ...
        // Crear un array de promesas para obtener los saldos de cada producto
        const promesasSaldos = detalleVenta.map(objDetalle => {
            let cantProducto = parseFloat($('#txtCant_idItem_' + objDetalle.idItem.toString()).val());
            return IntervencionSanitaria.FarmDevuelveSaldosSegunAlmacenProductoTipoSalida($('#cboFarmaciaVenta').val(), objDetalle.idItem, 2)
                .then(saldoAlmacen => ({ objDetalle, saldoAlmacen, cantProducto })); // Devolver los datos relevantes en la misma promesa
        });
        ...
        try {
            ...
            resultados.forEach(({ objDetalle, saldoAlmacen, cantProducto }) => {
                const { idItem, codigo, idTipoSalidaBienInsumoSaldo, producto, prVenta, total: totalProducto } = objDetalle;

                if (prVenta == 0) {
                    errorMensaje += `No hay PRECIO para ${codigo} - ${producto} \n`;
                } else if (!isEmpty(saldoAlmacen)) {
                    let productoDespacho = {
                        ...
                        precioDelSeguro: prVenta,
                        tipo: idTipoSalidaBienInsumoSaldo
                    };

                    // Calcular el total acumulado
                    total = (parseFloat(total) + cantProducto * prVenta).toFixed(2);

                    productosParaDespachar.push(productoDespacho); // mRs_Productos1
                } else {
                    errorMensaje += `No hay SALDO para ${codigo} - ${producto} \n`;
                }
            });

            // Aquí puedes manejar los productos despachados y otros resultados
        } catch (error) {
            alerta(2, "Error al obtener saldos")
        ...
        try {
            param = await IntervencionSanitaria.SeleccionaParametros(347)

            IntervencionSanitaria.oDoMovimiento.IdAlmacenOrigen = $('#cboFarmaciaVenta').val()

            if (IntervencionSanitaria.TipoAccion == 'M') {
                NotaSalidaAgregar = await IntervencionSanitaria.FarmMovimientoAgregarModificar(IntervencionSanitaria.oDoMovimiento.MovNumero, IntervencionSanitaria.oDoMovimiento.MovTipo, IntervencionSanitaria.oDoMovimiento.IdAlmacenOrigen,
                    IntervencionSanitaria.oDoMovimiento.IdAlmacenDestino, IntervencionSanitaria.oDoMovimiento.IdTipoConcepto, IntervencionSanitaria.oDoMovimiento.DocumentoIdTipo, IntervencionSanitaria.oDoMovimiento.DocumentoNumero,
                    null, null, null, null, null, null, null, null, null, null, null, null,
                    IntervencionSanitaria.oDoMovimiento.Observaciones, IntervencionSanitaria.oDoMovimiento.Total, 0, 0, JSON.stringify(farmMovimientoDetalle))
            }

            let productosConLotes = await IntervencionSanitaria.DevuelveDetalleProductosConLotes(IntervencionSanitaria.oDoMovimiento.IdAlmacenOrigen, productosParaDespachar, 1, param.valorTexto)
            ...
            for (let [i, objProductosConLotes] of productosConLotes.entries()) {
                ...
                let chequeSaldoOk = await IntervencionSanitaria.ChequeaQueSaldosConLotesSeaPositivo(IntervencionSanitaria.oDoMovimiento.IdAlmacenOrigen, objProductosConLotes.IdProducto, objProductosConLotes.Lote, objProductosConLotes.FechaVencimiento, objProductosConLotes.idTipoSalidaBienInsumo, objProductosConLotes.Cantidad)
                ...
                if (!chequeSaldoOk) {
                    ...
                    productosLotesParaDespachoIntervencionSanitaria.push(objProductosConLotes)
                }

            }

            if (IntervencionSanitaria.oDoMovimiento.MovNumero == 0) {
                lcCorrelativo = await IntervencionSanitaria.FarmDevuelveYactualizaCorrelativosDeDocumentosES(2)
            } else {
                lcCorrelativo = IntervencionSanitaria.oDoMovimiento.MovNumero
            }

            param = await IntervencionSanitaria.SeleccionaParametros(225)
            IntervencionSanitaria.oDoCajaNroDocumento.IdCaja = param.valorTexto

            param = await IntervencionSanitaria.SeleccionaParametros(226)
            IntervencionSanitaria.oDoCajaNroDocumento.IdTipoComprobante = param.valorTexto

            if (IntervencionSanitaria.oDoMovimiento.DocumentoNumero == '') {
                //datosCaja = await IntervencionSanitaria.ObtenerSiguienteNumeroDocumentoYGrabarlo(IntervencionSanitaria.oDoCajaNroDocumento.IdTipoComprobante, IntervencionSanitaria.oDoCajaNroDocumento.IdCaja)
                //IntervencionSanitaria.oDoCajaNroDocumento.NroDocumento = datosCaja.nroDocumento
                //lcTicket = datosCaja.nroSerie + '-' + IntervencionSanitaria.oDoCajaNroDocumento.NroDocumento

                const concepto = await IntervencionSanitaria.SeleccionarConcepto(22);
                lcTicket = parseInt(concepto.documentoUltimoNumero) + 1;
            } else {
                lcTicket = IntervencionSanitaria.oDoMovimiento.DocumentoNumero
            }

            IntervencionSanitaria.oDoMovimiento.MovNumero = lcCorrelativo
            IntervencionSanitaria.oDoMovimiento.DocumentoNumero = lcTicket
            IntervencionSanitaria.oDoMovimiento.Total = total

            //param = await IntervencionSanitaria.SeleccionaParametros(227)             //COMENTAOD - PARA VENTAS
            //IntervencionSanitaria.oDoMovimiento.DocumentoIdTipo = param.valorTexto        //COMENTADO - PARA VENTAS
            IntervencionSanitaria.oDoMovimiento.DocumentoIdTipo = 23      //PARA INTERVENCIONES SANITARIAS


            NotaSalidaAgregar = await IntervencionSanitaria.FarmMovimientoAgregarModificar(IntervencionSanitaria.oDoMovimiento.MovNumero, IntervencionSanitaria.oDoMovimiento.MovTipo, IntervencionSanitaria.oDoMovimiento.IdAlmacenOrigen,
                IntervencionSanitaria.oDoMovimiento.IdAlmacenDestino, IntervencionSanitaria.oDoMovimiento.IdTipoConcepto, IntervencionSanitaria.oDoMovimiento.DocumentoIdTipo, IntervencionSanitaria.oDoMovimiento.DocumentoNumero,
                null, null, null, null, null, null, null, null, null, null, null, null,
                IntervencionSanitaria.oDoMovimiento.Observaciones, IntervencionSanitaria.oDoMovimiento.Total, IntervencionSanitaria.oDoMovimiento.IdEstadoMovimiento, 0, JSON.stringify(farmMovimientoDetalle))

            let lcFiltro = "tipoAlmacen='F' AND tipoMov='S' AND tipoSuministro='01' AND DocumentoId = 23";
            let relMov = await IntervencionSanitaria.FarmRelModDevuelveSegunFiltro(lcFiltro);
            if (!isEmpty(relMov)) {
                await IntervencionSanitaria.FarmRelModActualizaSegunFiltro(lcFiltro, IntervencionSanitaria.oDoMovimiento.DocumentoNumero);
            }

            IntervencionSanitaria.oDoFarmMovimientoIntervencionSanitaria.FechaCreacion = fechaFormateada
            IntervencionSanitaria.oDoFarmMovimientoIntervencionSanitaria.movNumero = lcCorrelativo
            IntervencionSanitaria.oDoFarmMovimientoIntervencionSanitaria.idCuentaAtencion = $('#txtNroCuenta').val()
            IntervencionSanitaria.oDoFarmMovimientoIntervencionSanitaria.idDiagnostico = 0
            IntervencionSanitaria.oDoFarmMovimientoIntervencionSanitaria.idPrescriptor = $('#cboPrescriptor').val()
            IntervencionSanitaria.oDoFarmMovimientoIntervencionSanitaria.idTipoReceta = $('#cboTipoReceta').val()
            IntervencionSanitaria.oDoFarmMovimientoIntervencionSanitaria.FechaHoraPrescribe = $('#txtFechaPrescripcion').val() + ' ' + $('#txtHoraPrescripcion').val()

            if (IntervencionSanitaria.oDoFarmMovimientoIntervencionSanitaria.idCuentaAtencion > 0) {
                let datosGenerales = await IntervencionSanitaria.AtencionesSelecionarPorCuenta(IntervencionSanitaria.oDoFarmMovimientoIntervencionSanitaria.idCuentaAtencion)
            ...
            //let venta = await IntervencionSanitaria.FarmMovimientoVentasAgregar(IntervencionSanitaria.oDoFarmMovimientoIntervencionSanitaria)

            //if (venta > 0) {

            //    //for (let objProductosConLotes of productosLotesParaDespachoIntervencionSanitaria) { // se debe validar los productos que no tienen stock

            //    //    //let chequeSaldoOk = await NotaSalida.ChequeaQueSaldosConLotesSeaPositivo(FarmMovimiento.IdAlmacenOrigen, objProductosConLotes.IdProducto, objProductosConLotes.Lote, objProductosConLotes.FechaVencimiento, objProductosConLotes.idTipoSalidaBienInsumo, objProductosConLotes.Cantidad)

            //    //    //if (!chequeSaldoOk) {
            //    //    //    errorMensaje += `Ya no hay Saldos para el Producto ${objProductosConLotes.Codigo} - ${objProductosConLotes.NombreProducto} \n`
            //    //    //}
            //    //    let detalleVenta = await IntervencionSanitaria.FarmMovimientoVentasDetalleAgregarV2(IntervencionSanitaria.oDoFarmMovimientoIntervencionSanitaria.movNumero, IntervencionSanitaria.oDoFarmMovimientoIntervencionSanitaria.MovTipo, objProductosConLotes) // falta este producto
            //    //}

            //    let lbEsPlanAtencion = false
            //    let lbGeneraPago = await IntervencionSanitaria.TiposFinanciamientoGeneraReciboPago(IntervencionSanitaria.oDoFarmMovimientoIntervencionSanitaria.IdTipoFinanciamiento)
            //    let TipoFinanciamiento = await IntervencionSanitaria.ConsultaBloqueaTablaXidTipoFinanciamiento(IntervencionSanitaria.oDoFarmMovimientoIntervencionSanitaria.IdTipoFinanciamiento)

            //    if (TipoFinanciamiento.esFuenteFinanciamiento) {
            //        lbEsPlanAtencion = true
            //    } else {
            //        lbEsPlanAtencion = false
            //        if (TipoFinanciamiento.idTipoFinanciamiento == 9) { // Servicio Social genera datos en tablas de FACTURACION
            //            lbEsPlanAtencion = true
            //            lbGeneraPago = 0
            //        }
            //    }
            ...
            //    if (lbGeneraPago == 1) {
            //        oDoFactOrdenesBienes = {
            //            FechaCreacion: IntervencionSanitaria.oDoFarmMovimientoIntervencionSanitaria.FechaCreacion,
            //            IdCuentaAtencion: IntervencionSanitaria.oDoFarmMovimientoIntervencionSanitaria.idCuentaAtencion,
            //            IdEstadoFacturacion: 1,
            //            IdPaciente: IntervencionSanitaria.oDoFarmMovimientoIntervencionSanitaria.IdPaciente,
            //            idPreVenta: 0,
            //            IdPuntoCarga: 5,
            //            MovNumero: IntervencionSanitaria.oDoFarmMovimientoIntervencionSanitaria.movNumero,
            //            MovTipo: IntervencionSanitaria.oDoFarmMovimientoIntervencionSanitaria.MovTipo
            //        }

            //        // funcion para guardar ordenes bienes
            //        oDoFactOrdenesBienes.IdOrden = await IntervencionSanitaria.FactOrdenesBienesAgregarVentas(oDoFactOrdenesBienes)

            //    }

            //    let oDoFacturacionBienesFinanciam = {}
            //    let oDoFacturacionBienesPagos = {}

            //    oDoFacturacionBienesFinanciam.IdTipoFinanciamiento = IntervencionSanitaria.oDoFarmMovimientoIntervencionSanitaria.IdTipoFinanciamiento
            //    oDoFacturacionBienesFinanciam.FechaAutoriza = IntervencionSanitaria.oDoFarmMovimientoIntervencionSanitaria.FechaCreacion
            //    oDoFacturacionBienesFinanciam.MovNumero = IntervencionSanitaria.oDoMovimiento.MovNumero
            //    oDoFacturacionBienesFinanciam.MovTipo = IntervencionSanitaria.oDoMovimiento.MovTipo
            //    oDoFacturacionBienesFinanciam.idFuenteFinanciamiento = IntervencionSanitaria.oDoFarmMovimientoIntervencionSanitaria.idFuenteFinanciamiento

            //    for (let oRsDetalleProductos of productosParaDespachar) { // se debe validar los productos que no tienen stock

            //        //if (lbGeneraPago == 0 && lbEsPlanAtencion) {
            //        //    oDoFacturacionBienesFinanciam.CantidadFinanciada = oRsDetalleProductos.cantidad
            //        //    oDoFacturacionBienesFinanciam.idProducto = oRsDetalleProductos.idProducto
            //        //    oDoFacturacionBienesFinanciam.PrecioFinanciado = oRsDetalleProductos.precioDelSeguro
            //        //    oDoFacturacionBienesFinanciam.TotalFinanciado = (parseFloat(oRsDetalleProductos.precioDelSeguro) * parseFloat(oRsDetalleProductos.cantidad)).toFixed(2)

            //        //    //oFacturacionBienesFinanciam.Insertar(oDoFacturacionBienesFinanciam)
            //        //    let factBienesFinanciamiento = await IntervencionSanitaria.FacturacionBienesFinanciamientosAgregar(oDoFacturacionBienesFinanciam.MovNumero, oDoFacturacionBienesFinanciam.MovTipo, oDoFacturacionBienesFinanciam.idProducto,
            //        //        oDoFacturacionBienesFinanciam.IdTipoFinanciamiento, oDoFacturacionBienesFinanciam.idFuenteFinanciamiento, oDoFacturacionBienesFinanciam.CantidadFinanciada, oDoFacturacionBienesFinanciam.PrecioFinanciado, oDoFacturacionBienesFinanciam.TotalFinanciado)
            //        //}

            //        if (lbGeneraPago == 1 && lbEsPlanAtencion) {
            ...
            //            let factBienesFinanciamiento = await IntervencionSanitaria.FacturacionBienesPagosAgregar(oDoFacturacionBienesPagos.IdOrden, oDoFacturacionBienesPagos.idProducto, oDoFacturacionBienesPagos.CantidadPagar, oDoFacturacionBienesPagos.PrecioVenta, oDoFacturacionBienesPagos.TotalPagar)

            //        }
            //    }

            //    if (IntervencionSanitaria.idRecetaProcesada > 0) {
            ...
            //            puntoCarga: "",
            //            idCuentaAtencion: null,
            //            idReceta: IntervencionSanitaria.idRecetaProcesada,
            //            fechaReceta: "",
            //            servicio: "",
            //            idPuntoCarga: null,
            //            idFormaPago: null,
            ...
            //        let recetaCabecera = await IntervencionSanitaria.RecetaCabeceraPoridReceta(oDoRecetaCabecera.idReceta)

            //        if (recetaCabecera.length > 0) {

            //            oDoRecetaCabecera = recetaCabecera[0]

            //            if (IntervencionSanitaria.oDoMovimiento.IdEstadoMovimiento == 0) {
            //                oDoRecetaCabecera.idEstado = 1
            //            } else {
            //                oDoRecetaCabecera.idEstado = 2
            //            }

            //            let cabeceraModificar = await IntervencionSanitaria.RecetaCabeceraModificar(oDoRecetaCabecera)

            //            let cabceraDetalle = await IntervencionSanitaria.RecetaCabeceraDetalleSeleccionaPorNroReceta(oDoRecetaCabecera.idReceta)

            //            for (let objCabceraDetalle of cabceraDetalle) {


            //                let recetaDespacho = await IntervencionSanitaria.RecetaDetalleActualizaCantDespachada(idReceta = oDoRecetaCabecera.idReceta, idItem = objCabceraDetalle.idItem, CantidadDespachada = objCabceraDetalle.cantidadPedida, IdEstadoDetalle = 2)
            //                let recetaDetalleItem = await IntervencionSanitaria.RecetaDetalleItemAgregar(idReceta = oDoRecetaCabecera.idReceta, idItem = objCabceraDetalle.idItem, DocumentoDespacho = lcTicket, CantidadDespachada = objCabceraDetalle.cantidadPedida)

            //            }

            //            if (IntervencionSanitaria.TipoAccion == 'A') {
            //                let relacionOrdenPago = IntervencionSanitaria.FarmRecetaRelacionOrdenPagoAgregar(oDoRecetaCabecera.idReceta, 0, lcTicket, 2)
            //            }
            ...
            //    IntervencionSanitaria.farmMovimientoProgramas.movNumero = IntervencionSanitaria.oDoMovimiento.MovNumero;
            //    IntervencionSanitaria.farmMovimientoProgramas.movTipo = 'S';
            //    IntervencionSanitaria.farmMovimientoProgramas.idPrescriptor = $("#cboPrescriptor").val();
            //    IntervencionSanitaria.farmMovimientoProgramas.idCoordinador = $("#cboCoordinadorIntSanit").val();
            //    IntervencionSanitaria.farmMovimientoProgramas.idComponente = $("#cboComponenteIntSanit").val();
            //    IntervencionSanitaria.farmMovimientoProgramas.idSubComponente = $("#cboSubComponenteIntSanit").val();
            //    IntervencionSanitaria.farmMovimientoProgramas.idDiagnostico = $("#cboDiagnosticoIntSanit").val();
            //    IntervencionSanitaria.farmMovimientoProgramas.fechaHoraPrescribe = $("#txtFechaPrescripcion").val() + ' ' + $("#txtHoraPrescripcion").val();
            //    IntervencionSanitaria.farmMovimientoProgramas.idPaciente = IntervencionSanitaria.oDoFarmMovimientoIntervencionSanitaria.IdPaciente;
            //    IntervencionSanitaria.farmMovimientoProgramas.idCuentaAtencion = IntervencionSanitaria.oDoFarmMovimientoIntervencionSanitaria.idCuentaAtencion;
            //    IntervencionSanitaria.farmMovimientoProgramas.observaciones = $("#txtObservacionesIntSanit").val();
            //    let intSanit = await IntervencionSanitaria.FarmMovimientoProgramasGuardar(IntervencionSanitaria.farmMovimientoProgramas);
            ...
            //        textoAlerta = 'Se Anuló Nª ' + IntervencionSanitaria.oDoMovimiento.MovNumero + 'F'
            //    } else if (IntervencionSanitaria.TipoAccion == 'A') {
            //        textoAlerta = "Se Agregó la Guia Interna de Salida Nº " + IntervencionSanitaria.oDoMovimiento.MovNumero
            //        let url = "/Farmacias/ImpreInformeVentas?area=ConsultaExterna&MovNumero=" + IntervencionSanitaria.oDoMovimiento.MovNumero + "&MovTipo=" + 'S'
            //        $('#ifrmInforme').attr('src', url);
            //        $("#modalInforme").modal('show');
            //    } else if (IntervencionSanitaria.TipoAccion == 'M') {
            //        textoAlerta = "Se Modificó la Guia Interna de Salida Nº " + IntervencionSanitaria.oDoMovimiento.MovNumero
            //        let url = "/Farmacias/ImpreInformeVentas?area=ConsultaExterna&MovNumero=" + IntervencionSanitaria.oDoMovimiento.MovNumero + "&MovTipo=" + 'S'
            //        $('#ifrmInforme').attr('src', url);
            //        $("#modalInforme").modal('show');
            //    }
            ...
            IntervencionSanitaria.farmMovimientoProgramas.idCuentaAtencion = IntervencionSanitaria.oDoFarmMovimientoIntervencionSanitaria.idCuentaAtencion;
            IntervencionSanitaria.farmMovimientoProgramas.observaciones = $("#txtObservacionesIntSanit").val();
            IntervencionSanitaria.farmMovimientoProgramas.nroFormato = $("#txtNroFormatoIntSanit").val();
            let intSanit = await IntervencionSanitaria.FarmMovimientoProgramasGuardar(IntervencionSanitaria.farmMovimientoProgramas);
            ...
            if (IntervencionSanitaria.TipoAccion == 'E') {
                //textoAlerta = 'Se Anuló Nª ' + IntervencionSanitaria.oDoMovimiento.MovNumero + 'F'
                textoAlerta = "Se Anuló la Guia Interna de Salida Nª " + lcTicket
            } else if (IntervencionSanitaria.TipoAccion == 'A') {
                //textoAlerta = "Se Agregó la Guia Interna de Salida Nº " + IntervencionSanitaria.oDoMovimiento.MovNumero
                textoAlerta = "Se Agregó la Guia Interna de Salida Nº " + lcTicket
                let url = "/Farmacias/ImpreInformeVentas?area=ConsultaExterna&MovNumero=" + IntervencionSanitaria.oDoMovimiento.MovNumero + "&MovTipo=" + 'S'
                $('#ifrmInforme').attr('src', url);
                $("#modalInforme").modal('show');
            } else if (IntervencionSanitaria.TipoAccion == 'M') {
                //textoAlerta = "Se Modificó la Guia Interna de Salida Nº " + IntervencionSanitaria.oDoMovimiento.MovNumero
                textoAlerta = "Se Modificó la Guia Interna de Salida Nº " + lcTicket
                let url = "/Farmacias/ImpreInformeVentas?area=ConsultaExterna&MovNumero=" + IntervencionSanitaria.oDoMovimiento.MovNumero + "&MovTipo=" + 'S'
                $('#ifrmInforme').attr('src', url);
                $("#modalInforme").modal('show');
            }
        ...
        //////        let saldoAlmacen = await IntervencionSanitaria.FarmDevuelveSaldosSegunAlmacenProducto($('#cboFarmaciaVenta').val(), objDetalle.idItem)
        ...
        //////            total = (parseFloat(total) + parseFloat(cantProducto) * parseFloat(objDetalle.prVenta).toFixed(3)).toFixed(2)
    ...
    async AgregarDatosDePreVenta() {
        ...
        for (let objDetalle of detalleVenta) {
            ...
            if (objDetalle.prVenta == 0) {
                ...
                // Validamos si la farmaia Desino tiene stock para realizar el desapcho UNIDOSIS
                let saldoAlmacen = await IntervencionSanitaria.FarmDevuelveSaldosSegunAlmacenProductoTipoSalida($('#cboFarmaciaVenta').val(), objDetalle.idItem, 2)
                ...
                if (!isEmpty(saldoAlmacen)) {
                    ...
                    total = (parseFloat(total) + IntervencionSanitaria.customRoundToTwoDecimals(parseFloat(cantProducto) * parseFloat(objDetalle.prVenta))).toFixed(2)
        ...
        IntervencionSanitaria.oDoPreventa.idAlmacen = $('#cboFarmaciaVenta').val()
        IntervencionSanitaria.oDoPreventa.total = total

        IntervencionSanitaria.oDoPreventa.fechaHoraPrescribe = $('#txtFechaPrescripcion').val() + ' ' + $('#txtHoraPrescripcion').val()
        IntervencionSanitaria.oDoPreventa.idTipoFinanciamiento = 1


        let productosConLotes = await IntervencionSanitaria.DevuelveDetalleProductosConLotes(IntervencionSanitaria.oDoPreventa.idAlmacen, productosParaDespachar, 1, '')
        ...
        for (let [i, objProductosConLotes] of productosConLotes.entries()) {
            ...
            //let chequeSaldoOk = await IntervencionSanitaria.ChequeaQueSaldosConLotesSeaPositivo(IntervencionSanitaria.oDoPreventa.idAlmacen, objProductosConLotes.IdProducto, objProductosConLotes.Lote, objProductosConLotes.FechaVencimiento, objProductosConLotes.idTipoSalidaBienInsumo, cantProducto)
            let chequeSaldoOk = await IntervencionSanitaria.ChequeaQueSaldosConLotesSeaPositivo(IntervencionSanitaria.oDoPreventa.idAlmacen, objProductosConLotes.IdProducto, objProductosConLotes.Lote, objProductosConLotes.FechaVencimiento, objProductosConLotes.idTipoSalidaBienInsumo, objProductosConLotes.Cantidad)
        ...
        let idPreVenta = await IntervencionSanitaria.FarmPreVentaAgregar()


        let oDoFactOrdenesBienes = {
            FechaCreacion: fechaFormateada,
            IdCuentaAtencion: IntervencionSanitaria.oDoPreventa.idcuentaAtencion,
            IdEstadoFacturacion: IntervencionSanitaria.oDoPreventa.idEstadoPreventa,
            IdPaciente: IntervencionSanitaria.oDoPreventa.idPaciente,
        ...
        // funcion para guardar ordenes bienes
        oDoFactOrdenesBienes.IdOrden = await IntervencionSanitaria.FactOrdenesBienesAgregar(oDoFactOrdenesBienes)
        ...
        for (let [i, oRsDetalleProductos] of productosParaDespachar.entries()) { // se debe validar los productos que no tienen stock
            ...
            let farmtPreVenta = await IntervencionSanitaria.FarmPreVentaDetalleAgregar(oDoPreventaDetalle.idPreVenta, oDoPreventaDetalle.idProducto, oDoPreventaDetalle.item, oDoPreventaDetalle.cantidad, oDoPreventaDetalle.precio, oDoPreventaDetalle.importe)
        ...
        if (IntervencionSanitaria.TipoAccion == 'A') {
            if ($('#txtNroReceta').val() != '') {
                let relacionOrdenPago = IntervencionSanitaria.FarmRecetaRelacionOrdenPagoAgregar($('#txtNroReceta').val(), idPreVenta, 0, 1)
            }
        ...
        if (IntervencionSanitaria.TipoAccion == 'E') {
            ...
            textoAlerta = "Se Agregó correctamente la Preventa <br> Tiene que ir a CAJA a cancelar con el Nª " + idPreVenta + 'F <br> Monto a Pagar: ' + Math.round(IntervencionSanitaria.oDoPreventa.total * 10) / 10;
        } else if (IntervencionSanitaria.TipoAccion == 'M') {
            textoAlerta = "Se Modificó correctamente la Preventa <br> Tiene que ir a CAJA a cancelar con el Nª " + idPreVenta + 'F <br> Monto a Pagar: ' + Math.round(IntervencionSanitaria.oDoPreventa.total * 10) / 10;
        }
    ...
    async FarmPreVentaAgregar() {
        ...
        data.append('IdPreventa', IntervencionSanitaria.oDoPreventa.idPreventa);
        data.append('idAlmacen', IntervencionSanitaria.oDoPreventa.idAlmacen);
        data.append('idPaciente', IntervencionSanitaria.oDoPreventa.idPaciente);
        data.append('idTipoFinanciamiento', IntervencionSanitaria.oDoPreventa.idTipoFinanciamiento);
        data.append('total', IntervencionSanitaria.oDoPreventa.total);
        data.append('idDiagnostico', IntervencionSanitaria.oDoPreventa.idDiagnostico);
        data.append('idTipoReceta', IntervencionSanitaria.oDoPreventa.idTipoReceta);
        data.append('idcuentaAtencion', IntervencionSanitaria.oDoPreventa.idcuentaAtencion);
        data.append('idPrescriptor', IntervencionSanitaria.oDoPreventa.idPrescriptor);
        data.append('idEstadoPreventa', IntervencionSanitaria.oDoPreventa.idEstadoPreventa);
        data.append('fechaHoraPrescribe', IntervencionSanitaria.oDoPreventa.fechaHoraPrescribe);
        data.append('dni', $('#txtNroDocumentoPaciente').val().length <= 8 ? $('#txtNroDocumentoPaciente').val() : $('#txtNroDocumentoPaciente').val().substr(2, $('#txtNroDocumentoPaciente').val().length));
    ...
    async FarmMovimientoProgramasGuardar(farmMovimientoPrograma) {
        //da
        var respuesta;
        var resp = false;
        let datos
        ...
        data.append('MovNumero', farmMovimientoPrograma.movNumero);
        data.append('MovTipo', farmMovimientoPrograma.movTipo);
        data.append('IdCoordinador', farmMovimientoPrograma.idCoordinador);
        data.append('IdPrescriptor', farmMovimientoPrograma.idPrescriptor);
        data.append('IdDiagnostico', farmMovimientoPrograma.idDiagnostico);
        data.append('IdPaciente', farmMovimientoPrograma.idPaciente);
        data.append('IdComponente', farmMovimientoPrograma.idComponente);
        data.append('IdSubComponente', farmMovimientoPrograma.idSubComponente);
        data.append('FechaHoraPrescribe', farmMovimientoPrograma.fechaHoraPrescribe);
        data.append('IdCuentaAtencion', farmMovimientoPrograma.idCuentaAtencion);
        data.append('Observaciones', farmMovimientoPrograma.observaciones);
        data.append('NroFormato', farmMovimientoPrograma.nroFormato);
        ...
        try {
            ...
            datos = await
                $.ajax({
                    method: "POST",
                    url: "/Farmacias/FarmMovimientoProgramasGuardar?area=Farmacia",
                    //contentType: "application/json; charset=utf-8",
                    data: data,
                    dataType: "json",
                    cache: false,
    ...
    async CargarDatosVentaDirecta(movNumero, idEstado) {
        ...
        if (IntervencionSanitaria.TipoAccion != 'A') {
            ...
            console.log('farmMovimiento', farmMovimiento)
            console.log('farmMovimientoVentas', farmMovimientoIntervencionSanitaria)
            console.log('atencionCuenta', atencionCuenta)


            IntervencionSanitaria.oDoMovimiento = {
                FechaCreacion: farmMovimiento.fechaCreacion,
                MovNumero: farmMovimiento.movNumero,
                DocumentoNumero: farmMovimiento.documentoNumero,
                DocumentoIdTipo: farmMovimiento.documentoIdtipo,
                IdAlmacenDestino: farmMovimiento.idAlmacenDestino, // Almacen -- 0 -> Ninguno
                IdAlmacenOrigen: farmMovimiento.idAlmacenOrigen,
                IdEstadoMovimiento: idEstado,
                IdTipoConcepto: farmMovimiento.idTipoConcepto,
                MovTipo: farmMovimiento.movTipo,
                Observaciones: farmMovimiento.observaciones,
                Total: farmMovimiento.total
            }

            IntervencionSanitaria.oDoFarmMovimientoIntervencionSanitaria = {
                idCuentaAtencion: farmMovimientoIntervencionSanitaria.idCuentaAtencion,
                idDiagnostico: farmMovimientoIntervencionSanitaria.idDiagnostico,
                IdPaciente: farmMovimientoIntervencionSanitaria.idPaciente,
                idPrescriptor: farmMovimientoIntervencionSanitaria.idPrescriptor,
                IdTipoFinanciamiento: farmMovimientoIntervencionSanitaria.idTipoFinanciamiento,
                idTipoReceta: farmMovimientoIntervencionSanitaria.idTipoReceta,
                IdUsuarioAuditoria: null,
                MovTipo: farmMovimientoIntervencionSanitaria.movTipo,
                tipoVenta: farmMovimientoIntervencionSanitaria.tipoVenta,
                idFuenteFinanciamiento: farmMovimientoIntervencionSanitaria.idFuenteFinanciamiento,
                FechaHoraPrescribe: farmMovimientoIntervencionSanitaria.fechaHoraPrescribe,
                IdPaquete: farmMovimientoIntervencionSanitaria.idPaquete,
                idPreVenta: farmMovimientoIntervencionSanitaria.idPreVenta,
                movNumero: farmMovimientoIntervencionSanitaria.movNumero,
                idServicioPaciente: farmMovimientoIntervencionSanitaria.idServicioPaciente,
                FechaCreacion: null
            }
            ...
            //    let saldoAlmacen = await IntervencionSanitaria.FarmDevuelveSaldosSegunAlmacenProducto($('#cboFarmaciasBusq').val(), objCabceraDetalle.idProducto)
            ...
            for (let objCabceraDetalle of farmMovimientoDetalle) {
                ...
                //    if (data && data.idItem === objCabceraDetalle.idProducto) {
                //        // Si el producto ya existe, actualizar su cantidad, precio y total



                //        data.cantidadPedida += objCabceraDetalle.cantidad;
    ...
    async CargarDatosPreventa(idPreventa, idEstado) {
        ...
        if (IntervencionSanitaria.TipoAccion != 'A') {
            ...
            let preVenta = await IntervencionSanitaria.FarmPreVentaSeleccionarPorId(idPreventa)

            let farmMovimientoDetalle = await IntervencionSanitaria.FarmPreVentaDetalleDevuelveTodosItems(idPreventa)

            let atencionCuenta = await IntervencionSanitaria.AtencionesSelecionarPorCuenta(preVenta.idCuentaAtencion)
            ...
            await IntervencionSanitaria.TipoVenta_Change(2);
            ...
            let intervSanit = await IntervencionSanitaria.FarmMovimientoProgramasSeleccionar(preVenta.MovNumero);
            if (!isEmpty(intervSanit)) {
                $('#cboCoordinadorIntSanit').val(intervSanit.idCoordinador);
                $('#cboComponenteIntSanit').val(intervSanit.idComponente);
                $('#cboComponenteIntSanit').change();
                $('#cboSubComponenteIntSanit').val(intervSanit.idSubComponente);
                await Ordenes.ListaDiagnosticosIntervencionSanitaria(intervSanit.codigoDx);
                $('#cboDiagnosticoIntSanit').val(intervSanit.idDiagnostico);
                $('#txtObservacionesIntSanit').val(intervSanit.observaciones);
                $('#txtNroFormatoIntSanit').val(intervSanit.nroFormato);
            }
