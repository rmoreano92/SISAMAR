...
var Kardex = {
    ...
    Eventos() {
        $('#txtDescripcion').on('input', async function () {
            let inputValue = $(this).val()

            var midata = new FormData();

            midata.append('codigo', $("#txtCodigo").val());
            midata.append('descripcion', inputValue);
            ...
            if (inputValue.length > 3) {
                ...
                try {
                    ...
                    if (datos.table.length > 0) {
                        ...
                        console.log('datos.table', datos.table)

                        oTable_ProductosBuscados.fnAddData(datos.table)
        ...
        $('#tblProductosBuscados tbody').on('dblclick', 'tr', function () {
            ...
            $("#txtDescripcion").val("(" + $("#txtCodigo").val() + ") " + row.nombre);
            $("#hdIdProducto").val(datos.table[0].idProducto);
            Variables.IdProductoBienInsumo = row.idProducto;
        ...
        $('#btnGenerarReporte').on('click', () => {
            ...
            fetch('/Kardex/rptKardex?area=Farmacia', {
                method: "POST",
                body: midata
            })
    ...
    initDatables() {
                ...
                },
                {
                    ...
                    createdCell: async function (td, cellData, rowData, row, col) {
                        ...
                        if (await Utilitario.ValidarPermiso(302)) {
                            botonActualizarFechaVenc = "<button class='btn btn-sm btn-outline-info ml-1 btnActualizarFechaVencimiento' type='button'><i class='fa-solid fa-pencil'></i></button>"
                        }                        
        ...
        var tableWrapper = $('#tblKardex'); // datatable creates the table wrapper by adding with id {your_table_id}_wrapper
        //tableWrapper.find('.dataTables_length select').select2(); // initialize select2 dropdown
        oTable_Kardex = $("#tblKardex").dataTable(parms);
    ...
    async ListarKardex() {        
        ...
        var descripcion1 = "<div class='mr-2'><b>Almacen:</b> " + $("#cboAlmacen option:selected").text() + "</div> <div class='mr-2'><b>F.Movimiento:</b> " + $("#txtFechaInicio").val() + " al " + $("#txtFechaFin").val() + "</div> <div><b>Tipo Salida:</b> " + $("#cboTipoSalida option:selected").text() + "</div>";
...
$(document).ready(function () {
    Kardex.cargaInicial();
    Kardex.initDatables();
    Kardex.DataTableProductosBuscados();
});
