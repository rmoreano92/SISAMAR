var Ventas = {
    ...
    async FarmMovimientoVentasAgregar(FarmMovimientoVentas) {
        ...
        data.append('PresExternoCmp', FarmMovimientoVentas.PresExternoCmp);
        data.append('PresExternoMedico', FarmMovimientoVentas.PresExternoMedico);
        data.append('PresExternoFecha', FarmMovimientoVentas.PresExternoFecha);

        data.append('NroFormato', FarmMovimientoVentas.NroFormato);
        //data.append('detalleNotaSalida', detalleNotaSalida);
    ...
    async AgregarDatosDeVentaDirecta() {
        ...
        let productosLotesParaDespachoVentas = []
        ...
        try {
            ...
            const promesasSaldos = detalleVenta.map(objDetalle => {
                ...
                return Ventas.FarmDevuelveSaldosSegunAlmacenProductoTipoSalida($('#cboFarmaciaVenta').val(), objDetalle.idItem, 1)
        ...
        try {
            param = await Ventas.SeleccionaParametros(347)

            Ventas.oDoMovimiento.IdAlmacenOrigen = $('#cboFarmaciaVenta').val()

            if (Ventas.TipoAccion == 'M') {
                NotaSalidaAgregar = await Ventas.FarmMovimientoAgregarModificar(Ventas.oDoMovimiento.MovNumero, Ventas.oDoMovimiento.MovTipo, Ventas.oDoMovimiento.IdAlmacenOrigen,
                    Ventas.oDoMovimiento.IdAlmacenDestino, Ventas.oDoMovimiento.IdTipoConcepto, Ventas.oDoMovimiento.DocumentoIdTipo, Ventas.oDoMovimiento.DocumentoNumero,
                    null, null, null, null, null, null, null, null, null, null, null, null,
                    Ventas.oDoMovimiento.Observaciones, Ventas.oDoMovimiento.Total, 0, 0, JSON.stringify(farmMovimientoDetalle))
            }

            let productosConLotes = await Ventas.DevuelveDetalleProductosConLotes(Ventas.oDoMovimiento.IdAlmacenOrigen, productosParaDespachar, 1, param.valorTexto)
            ...
            for (let [i, objProductosConLotes] of productosConLotes.entries()) {
                ...
                let chequeSaldoOk = await Ventas.ChequeaQueSaldosConLotesSeaPositivo(Ventas.oDoMovimiento.IdAlmacenOrigen, objProductosConLotes.IdProducto, objProductosConLotes.Lote, objProductosConLotes.FechaVencimiento, objProductosConLotes.idTipoSalidaBienInsumo, objProductosConLotes.Cantidad)
                ...
                if (!chequeSaldoOk) {
                    ...
                    productosLotesParaDespachoVentas.push(objProductosConLotes)
                }
            ...
            if (Ventas.oDoMovimiento.MovNumero == 0) {
                lcCorrelativo = await Ventas.FarmDevuelveYactualizaCorrelativosDeDocumentosES(2)
            } else {
                lcCorrelativo = Ventas.oDoMovimiento.MovNumero
            }

            param = await Ventas.SeleccionaParametros(225)
            Ventas.oDoCajaNroDocumento.IdCaja = param.valorTexto

            param = await Ventas.SeleccionaParametros(226)
            Ventas.oDoCajaNroDocumento.IdTipoComprobante = param.valorTexto

            if (Ventas.oDoMovimiento.DocumentoNumero == '') {
                datosCaja = await Ventas.ObtenerSiguienteNumeroDocumentoYGrabarlo(Ventas.oDoCajaNroDocumento.IdTipoComprobante, Ventas.oDoCajaNroDocumento.IdCaja)
                if (!isEmpty(datosCaja)) {
                    Ventas.oDoCajaNroDocumento.NroDocumento = datosCaja.nroDocumento
                    lcTicket = datosCaja.nroSerie + '-' + Ventas.oDoCajaNroDocumento.NroDocumento
                } else {
                    alerta2("warning", "", "El número de documento no ha podido ser generado.");
                    return;
                }
            } else {
                lcTicket = Ventas.oDoMovimiento.DocumentoNumero
            }

            Ventas.oDoMovimiento.MovNumero = lcCorrelativo
            Ventas.oDoMovimiento.DocumentoNumero = lcTicket
            Ventas.oDoMovimiento.Total = total

            param = await Ventas.SeleccionaParametros(227)
            Ventas.oDoMovimiento.DocumentoIdTipo = param.valorTexto


            NotaSalidaAgregar = await Ventas.FarmMovimientoAgregarModificar(Ventas.oDoMovimiento.MovNumero, Ventas.oDoMovimiento.MovTipo, Ventas.oDoMovimiento.IdAlmacenOrigen,
                Ventas.oDoMovimiento.IdAlmacenDestino, Ventas.oDoMovimiento.IdTipoConcepto, Ventas.oDoMovimiento.DocumentoIdTipo, Ventas.oDoMovimiento.DocumentoNumero,
                null, null, null, null, null, null, null, null, null, null, null, null,
                Ventas.oDoMovimiento.Observaciones, Ventas.oDoMovimiento.Total, Ventas.oDoMovimiento.IdEstadoMovimiento, 0, JSON.stringify(farmMovimientoDetalle))

            Ventas.oDoFarmMovimientoVentas.FechaCreacion = fechaFormateada
            Ventas.oDoFarmMovimientoVentas.movNumero = lcCorrelativo
            Ventas.oDoFarmMovimientoVentas.idCuentaAtencion = $('#txtNroCuenta').val()
            Ventas.oDoFarmMovimientoVentas.idDiagnostico = 0
            Ventas.oDoFarmMovimientoVentas.idPrescriptor = $('#cboPrescriptor').val()
            Ventas.oDoFarmMovimientoVentas.idTipoReceta = $('#cboTipoReceta').val()
            Ventas.oDoFarmMovimientoVentas.FechaHoraPrescribe = $('#txtFechaPrescripcion').val() + ' ' + $('#txtHoraPrescripcion').val()

            Ventas.oDoFarmMovimientoVentas.PresExternoCmp = $('#txtCmpPrescriptorExterno').val()
            Ventas.oDoFarmMovimientoVentas.PresExternoMedico = $('#txtDatosMedicoExterno').val()
            Ventas.oDoFarmMovimientoVentas.PresExternoFecha = $('#txtFechaPrescripcionExterno').val() + ' ' + $('#txtHoraPrescripcionExt').val()

            Ventas.oDoFarmMovimientoVentas.NroFormato = $('#txtNroFormato').val()

            if (Ventas.oDoFarmMovimientoVentas.idCuentaAtencion > 0) {
                let datosGenerales = await Ventas.AtencionesSelecionarPorCuenta(Ventas.oDoFarmMovimientoVentas.idCuentaAtencion)

                if (!isEmpty(datosGenerales)) {
                    Ventas.oDoFarmMovimientoVentas.idServicioPaciente = $('#cboServicioRegistro').val()
                } else {
                    Ventas.oDoFarmMovimientoVentas.idServicioPaciente = 0
                }
            ...
            let venta = await Ventas.FarmMovimientoVentasAgregar(Ventas.oDoFarmMovimientoVentas)

                if (venta > 0) {

                    //for (let objProductosConLotes of productosLotesParaDespachoVentas) { // se debe validar los productos que no tienen stock
                    ...
                    //    //    errorMensaje += `Ya no hay Saldos para el Producto ${objProductosConLotes.Codigo} - ${objProductosConLotes.NombreProducto} \n`
                    //    //}
                    //    let detalleVenta = await Ventas.FarmMovimientoVentasDetalleAgregarV2(Ventas.oDoFarmMovimientoVentas.movNumero, Ventas.oDoFarmMovimientoVentas.MovTipo, objProductosConLotes) // falta este producto
                    //}

                    let lbEsPlanAtencion = false
                    let lbGeneraPago = await Ventas.TiposFinanciamientoGeneraReciboPago(Ventas.oDoFarmMovimientoVentas.IdTipoFinanciamiento)
                    let TipoFinanciamiento = await Ventas.ConsultaBloqueaTablaXidTipoFinanciamiento(Ventas.oDoFarmMovimientoVentas.IdTipoFinanciamiento)
                    ...
                    if (lbGeneraPago == 1) {
                        oDoFactOrdenesBienes = {
                            FechaCreacion: Ventas.oDoFarmMovimientoVentas.FechaCreacion,
                            IdCuentaAtencion: Ventas.oDoFarmMovimientoVentas.idCuentaAtencion,
                            IdEstadoFacturacion: 1,
                            IdPaciente: Ventas.oDoFarmMovimientoVentas.IdPaciente,
                            idPreVenta: 0,
                            IdPuntoCarga: 5,
                            MovNumero: Ventas.oDoFarmMovimientoVentas.movNumero,
                            MovTipo: Ventas.oDoFarmMovimientoVentas.MovTipo
                        }

                        // funcion para guardar ordenes bienes
                        oDoFactOrdenesBienes.IdOrden = await Ventas.FactOrdenesBienesAgregarVentas(oDoFactOrdenesBienes)
                    }

                    let oDoFacturacionBienesFinanciam = {}
                    let oDoFacturacionBienesPagos = {}

                    oDoFacturacionBienesFinanciam.IdTipoFinanciamiento = Ventas.oDoFarmMovimientoVentas.IdTipoFinanciamiento
                    oDoFacturacionBienesFinanciam.FechaAutoriza = Ventas.oDoFarmMovimientoVentas.FechaCreacion
                    oDoFacturacionBienesFinanciam.MovNumero = Ventas.oDoMovimiento.MovNumero
                    oDoFacturacionBienesFinanciam.MovTipo = Ventas.oDoMovimiento.MovTipo
                    oDoFacturacionBienesFinanciam.idFuenteFinanciamiento = Ventas.oDoFarmMovimientoVentas.idFuenteFinanciamiento
                    ...
                    for (let oRsDetalleProductos of productosParaDespachar) { // se debe validar los productos que no tienen stock
                        ...
                        //    let factBienesFinanciamiento = await Ventas.FacturacionBienesFinanciamientosAgregar(oDoFacturacionBienesFinanciam.MovNumero, oDoFacturacionBienesFinanciam.MovTipo, oDoFacturacionBienesFinanciam.idProducto,
                    ...
                    if (Ventas.idRecetaProcesada > 0) {
                        ...
                        if (recetaCabecera.length > 0) {
                            ...
                            //if (Ventas.oDoMovimiento.IdEstadoMovimiento == 0) {
                ...
                if (Ventas.TipoAccion == 'E') {
                    textoAlerta = 'Se Anuló Nª ' + Ventas.oDoMovimiento.MovNumero + 'F'
                } else if (Ventas.TipoAccion == 'A') {
                    textoAlerta = "Se Agregó venta N° " + Ventas.oDoMovimiento.MovNumero
                    let url = "/Farmacias/ImpreInformeVentas?area=ConsultaExterna&MovNumero=" + Ventas.oDoMovimiento.MovNumero + "&MovTipo=" + 'S'
                    $('#ifrmInforme').attr('src', url);
                    $("#modalInforme").modal('show');
                } else if (Ventas.TipoAccion == 'M') {
                    textoAlerta = "Se Modificó venta N° " + Ventas.oDoMovimiento.MovNumero
                    let url = "/Farmacias/ImpreInformeVentas?area=ConsultaExterna&MovNumero=" + Ventas.oDoMovimiento.MovNumero + "&MovTipo=" + 'S'
                    $('#ifrmInforme').attr('src', url);
                    $("#modalInforme").modal('show');
                }
        ...
        //////        let saldoAlmacen = await Ventas.FarmDevuelveSaldosSegunAlmacenProducto($('#cboFarmaciaVenta').val(), objDetalle.idItem)
    ...
    async LimpiarFormularioVariables() {
        ...
        Ventas.oDoCajaNroDocumento = {
            IdCaja: 0,
            IdTipoComprobante: 0
        }

        Ventas.oDoMovimiento = {
            FechaCreacion: '',
            MovNumero: 0,
            DocumentoNumero: '',
            DocumentoIdTipo: 0,
    ...
    async BuscarNumeroCuenta(idCuentaAtencion) {
        ...
        try {
            ...
            $('#txtDescripcionPlan').val("")
            $('.chzn-select').chosen().trigger("chosen:updated");
            Ventas.oDoFarmMovimientoVentas.IdPaciente = 0;
            Ventas.oDoFarmMovimientoVentas.idFuenteFinanciamiento = 0;
            Ventas.oDoFarmMovimientoVentas.IdTipoFinanciamiento = 0;
            Ventas.oDoFarmMovimientoVentas.idServicioPaciente = 0;
                ...
                swal({
                    ...
                    .then(async (res) => {
                        ...
                        if (res) {
                            ...
                            this.oDoMovimiento.IdTipoConcepto = fuenteFinanciamiento.idTipoConceptoFarmacia
    ...
    async CargarDatosVentaDirecta(movNumero, idEstado) {
        ...
        if (Ventas.TipoAccion != 'A') {
            ...
            //console.log('farmMovimientoVentas', farmMovimientoVentas)
            ...
            Ventas.oDoMovimiento = {
                ...
                PresExternoCmp: farmMovimiento.presExternoCmp,
                PresExternoMedico: farmMovimiento.presExternoMedico,
                PresExternoFecha: farmMovimiento.presExternoFecha
            }

            Ventas.oDoFarmMovimientoVentas = {
                idCuentaAtencion: farmMovimientoVentas.idCuentaAtencion,
                idDiagnostico: farmMovimientoVentas.idDiagnostico,
                IdPaciente: farmMovimientoVentas.idPaciente,
                idPrescriptor: farmMovimientoVentas.idPrescriptor,
                IdTipoFinanciamiento: farmMovimientoVentas.idTipoFinanciamiento,
                idTipoReceta: farmMovimientoVentas.idTipoReceta,
                IdUsuarioAuditoria: null,
                MovTipo: farmMovimientoVentas.movTipo,
                tipoVenta: farmMovimientoVentas.tipoVenta,
                idFuenteFinanciamiento: farmMovimientoVentas.idFuenteFinanciamiento,
                FechaHoraPrescribe: farmMovimientoVentas.fechaHoraPrescribe,
                IdPaquete: farmMovimientoVentas.idPaquete,
                idPreVenta: farmMovimientoVentas.idPreVenta,
                movNumero: farmMovimientoVentas.movNumero,
                idServicioPaciente: farmMovimientoVentas.idServicioPaciente,
                FechaCreacion: null,
                PresExternoCmp: farmMovimientoVentas.presExternoCmp,
                PresExternoMedico: farmMovimientoVentas.presExternoMedico,
                PresExternoFecha: farmMovimientoVentas.presExternoFecha,
                NroFormato: farmMovimientoVentas.nroFormato,
            }
            ...
            //    let saldoAlmacen = await Ventas.FarmDevuelveSaldosSegunAlmacenProducto($('#cboFarmaciasBusq').val(), objCabceraDetalle.idProducto)
