var NotaIngreso = {
    ...
    async GuardarMovimientoFarmacia() {
        ...
        if (NotaIngreso.Guardando == 0) {
            Cargando(1)
            NotaIngreso.Guardando = 1

            try {

                let idTipoLocales = $('#cboFarmaciaDestinoNI>option:selected').attr("idTipoLocales");
                let tipoSuministro = $('#cboFarmaciaDestinoNI>option:selected').attr("tipoSuministro");

                let detalleProductos = oTable_DetalleNI.api(true).data().toArray()
                let farmMovimientoDetalle = []
                $(detalleProductos).each(function (i, obj) {
                    console.log('obj', obj)

                    let cantProducto = $('#txtCant_idItem_' + obj.idProducto.toString()).val()
                ...
                if (NotaIngreso.TipoAccion == 'E') {
                    NotaIngreso.IdEstadoMovimiento = 0
                } else {
                    NotaIngreso.IdEstadoMovimiento = 1
                }

                let res = await NotaIngreso.CrearModificarNotaIngresoSalidaFarmacia(
                    MovNumero = NotaIngreso.MovNumero, MovTipo = 'S', idEstadoMovimiento = NotaIngreso.IdEstadoMovimiento, idTipoLocales = idTipoLocales,
                ...
                if (NotaIngreso.TipoAccion == 'E') {
                ...
                } else if (NotaIngreso.TipoAccion == 'A') {
                    ...
                    $('#ifrmInforme').attr('src', url);
                    $("#modalInforme").modal('show');
                } else if (NotaIngreso.TipoAccion == 'M') {
