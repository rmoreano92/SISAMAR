// JDELGADO002 CREACION
let RegistroPaciente = {

    initDataTable() {
        if (this.oTable_TablePacienteHijos) {
            this.oTable_TablePacienteHijos.destroy();
        }

        this.oTable_TablePacienteHijos = $("#DataTablePacienteHijos").DataTable({
            paging: true,
            ordering: true,
            searching: true,
            autoWidth: false,
            ...
            columns: [
                { data: 'paciente', title: 'Paciente', width: '30%' },
                { data: 'parentesco', title: 'Parentesco' },
                { data: 'nroHistoria', title: 'NroHistoria' },
                { data: 'estadoHistoria', title: 'EstadoHistoria' },
                { data: 'fechaNacimiento', title: 'FechaNacimiento' },
                { data: 'ultimoNumeroCuenta', title: 'UltimoNumeroCuenta' }
            ]
        });
    },

    async PacientesHijos_Listar(madreDocumento) {
        Cargando(1);
        //let madreDocumento = '';
        try {
            ...
            params.append('madreDocumento', madreDocumento) ?? '';

            const res = await HttpClient.Post('/Paciente/PacientesHijos_Listar?area=Comun', params);
            ...
            this.oTable_TablePacienteHijos.clear();

            if (res.data.table && res.data.table.length > 0) {
                console.log('Datos recibidos---------:', res.data.table[0]);
                //this.oTable_TablePacienteHijos.rows.add(res.data.table.draw());
                this.oTable_TablePacienteHijos.rows.add([res.data.table[0]]).draw();
    ...
    ListaParentesco: async () => {
        ...
        try {
            ...
            const $combo = $('#cboParentescoMadre');
            $combo.empty();
            $combo.append('<option value="">Seleccione una opción</option>');
        ...
        fetch('/Utilitario/ListaTiposDocumentos?area=Comun', {
            ...
            .then(response => {
                ...
                $(response.lsDocumentos.table).each(function (i, obj) {
                    ...
                    $('#cboTipoMPadres').append(`<option value="${obj.idDocIdentidad}">${obj.descripcionLarga}</option>`)
                    $('#cboTipoPPadres').append(`<option value="${obj.idDocIdentidad}">${obj.descripcionLarga}</option>`)
                })
    ...
    ListaPaises: async function () {
        ...
        $(res.data.table).each(function (i, obj) {
            ...
            $('#cboPaisMadre').append(`<option value="${obj.idPais}">${obj.nombre}</option>`)
        })
        $('.chzn-select').chosen().trigger("chosen:updated")
        $(`#cboPaisNacimiento`).val(166)
        $(`#cboPaisProcedencia`).val(166)
        $(`#cboPaisDomicilio`).val(166)
        $(`#cboPaisMadre`).val(166)
        ...
        await RegistroPaciente.ListaDepartamentosSeleccionarPorIdPais($('#cboPaisMadre').val(), 'cboDepartamentoMadre')
    ...
    ListaDepartamentos: async () => {
        ...
        await fetch('/Utilitario/ListaDepartamentos?area=Comun', {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
            }
            ...
            .then(response => {
                ...
                $('#cboDepartamentoProcedencia').empty();
                $('#cboDepartamentoNacimiento').empty();
                $('#cboDepartamentoMadre').empty();

                $('#cboDepartamentoDomicilio').append(`<option value="0">--Seleccionar--</option>`)
                $('#cboDepartamentoProcedencia').append(`<option value="0">--Seleccionar--</option>`)
                $('#cboDepartamentoNacimiento').append(`<option value="0">--Seleccionar--</option>`)
                $('#cboDepartamentoMadre').append(`<option value="0">--Seleccionar--</option>`)
                $(response.lsDeparta.table).each(function (i, obj) {
                    $('#cboDepartamentoDomicilio').append(`<option value="${obj.idDepartamento}">${obj.descripcionLarga}</option>`)
                    $('#cboDepartamentoProcedencia').append(`<option value="${obj.idDepartamento}">${obj.descripcionLarga}</option>`)
                    $('#cboDepartamentoNacimiento').append(`<option value="${obj.idDepartamento}">${obj.descripcionLarga}</option>`)
                    $('#cboDepartamentoMadre').append(`<option value="${obj.idDepartamento}">${obj.descripcionLarga}</option>`)
                })
    ...
    ListaDepartamentosSeleccionarPorIdPais: async (idPais, cboDepartamento) => {

        let formData = new FormData()

        formData.append('IdPais', idPais)

        await fetch('/Utilitario/ListaDepartamentosSeleccionarPorIdPais?area=Comun', {
            method: 'POST',
            //headers: {
            //    'Content-Type': 'application/json'
            //},
    ...
    ListaDistritosByProvincia: async (idDProvincia, cboDistrito) => {
        let formData = new FormData();
    ...
    ListaCentroPobladoByDistrito: async (idDistrito, cboCentroPoblado) => {
        let formData = new FormData();
    ...
    CompletarDatosPaciente: async function (data, tipoAccion) { // tipo accion sirve para identificar si se llama el metodo desde buscar datos paciente si es un buscara si es reingresante, nuevo continuador
        ...
        RegistroPaciente.PacientesHijos_Listar(data.nroDocumento)
        ...
        await RegistroPaciente.ListaProvinciasByDepartamentos(data.idDepartamentoMadre1, 'cboProvinciaMadre')
        await RegistroPaciente.ListaDistritosByProvincia(data.idProvinciaMadre1, 'cboDistritoMadre')
        await RegistroPaciente.ListaCentroPobladoByDistrito(data.idDistritoMadre1, 'cboCentroPobladoMadre')
        ...
        if (tipoAccion == 1) { // 1 = buscar
            ...
            let tipoPaciente = await RegistroPaciente.TipoPacienteEnEspecialidad(data.idPaciente,
                $('#cboEspecialidadCita').val() || $("#cboServicioEmergenciaBuscar option:checked")?.data("idespecialidad"))
            let planPaciente = await RegistroPaciente.BuscarFuentePorDNI(data.nroDocumento)
            ...
            if(planPaciente?.idFuenteFinanciamiento && planPaciente?.idTipoFinanciamiento){
                //AQUI SELECCIONA LA FUENTE DE FINANCIAMIENTO - CONSULTA EXTERNA
                $("#cboFuenteFinanciamientoCita")?.val(planPaciente.idFuenteFinanciamiento)?.chosen()?.trigger("chosen:updated").change();
                setTimeout(() => $("#cboProductoPlan")?.val(planPaciente.idTipoFinanciamiento)?.chosen()?.trigger("chosen:updated")?.change(), 500);
...
function limpiarCampos() {
    $("#txtApellidoPaternoContactoMadre").val('');
    $("#txtApellidoMaternoContactoMadre").val('');
    $("#txtPrimerNombreContactoMadre").val('');
    $("#txtTelefonoContactoMadre").val('');
    $("#chkParentescoContacto").prop("checked", false);
}
...
$(document).ready(() => {
    ...
    RegistroPaciente.ListaParentesco();
    ...
    RegistroPaciente.ListaProvinciasByDepartamentos($('#cboDepartamentoProcedencia').val(), 'cboProvinciaProcedencia')
    RegistroPaciente.ListaProvinciasByDepartamentos($('#cboDepartamentoNacimiento').val(), 'cboProvinciaNacimiento')
    RegistroPaciente.initDataTable();
    //RegistroPaciente.PacientesHijos_Listar()
