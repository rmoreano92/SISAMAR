var EcoUteroGravido = {
    ...
    async Iniciar() {
        EcoUteroGravido.Plugins();
        EcoUteroGravido.Eventos();
        await EcoUteroGravido.CargarCombos();
    ...
    Eventos() {
        ...
        $('.perfilBiofisico1').on("keyup", function (event) {    
            ...
            EcoUteroGravido.CalcularPerfilBiofisico(1);
        });
        ...
        $('.perfilBiofisico2').on("keyup", function (event) {
            ...
            EcoUteroGravido.CalcularPerfilBiofisico(2);
        });
        ...
        $('.perfilBiofisico3').on("keyup", function (event) {
            ...
            EcoUteroGravido.CalcularPerfilBiofisico(3);
        });

        $("#txtFechaFUR").on('change', async function () {
            let valor = $(this).val();

            await EcoUteroGravido.CalcularEdadGestacional(valor);
        });
    },
    ...
    async CargarCombos() {
        ...
        midata.append('catalogo', '36,37,38,39,40,41,42');
    ...
    async CargarResultado(idOrden, idMovimiento, idProducto) {
        ...
        if (isEmpty(resultados) == false) {
            resultados.forEach(function (valor) {
                ...
                $('#cboSexoFeto' + valor.numeroFeto).val(valor.idtipoSexoEco);
                ...
                $('#cboMorfFetalFeto' + valor.numeroFeto).val(valor.morfFetal);
    ...
    async SeleccionarResultadoEcografia(idOrden, idMovimiento, idProducto) {
        ...
        try {
            ...
            datos = await
                $.ajax({
                    method: "POST",
                    url: "/ImagenologiaEcografias/SeleccionarEcoUteroGravido?area=Imagenologia",
    ...
    DevolverDetalleResultado() {
        ...
        for (let i = 1; i <= fetos; i++) {
            objRes = {
                ...
                IdtipoSexoEco: $('#cboSexoFeto' + i).val(),
                ...
                MorfFetal: $('#cboMorfFetalFeto' + i).val(),
    ...
    async GuardarResultado() {
        ...
        try {
            ...
            datos = await
                $.ajax({
                    method: "POST",
                    url: "/ImagenologiaEcografias/GuardarEcoUteroGravido?area=Imagenologia",
                    //contentType: "application/json; charset=utf-8",
                    data: midata,
                    dataType: "json",
                    cache: false,
