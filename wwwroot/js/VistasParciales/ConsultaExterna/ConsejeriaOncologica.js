let ConsejeriaOncologica = {
    permisoClasiPaciente: '0',
    idOrden: 0,
    idOrdenPago: 0,

    async CargaInicial() {
        ConsejeriaOncologica.permisoClasiPaciente = await PermisoGeneral.SeleccionarPermisoGeneral("CLASI_PAC")
        if (ConsejeriaOncologica.permisoClasiPaciente == '0') {
            $('#divClasificacionPaciente').hide()
        } else {
            $('#divClasificacionPaciente').show()
        }

        let AtencionesCpt = await ConsejeriaOncologica.BuscaAtencionesCptCEparaFormatoHIS(Variables.IdCuentaAtencion)

        if (AtencionesCpt.listaCpt.table.length > 0) {
            ConsejeriaOncologica.idOrden = AtencionesCpt.listaCpt.table[0].idOrden
            ConsejeriaOncologica.idOrdenPago = AtencionesCpt.listaCpt.table[0].idOrdenPago
        } else {
            ConsejeriaOncologica.idOrden = 0
            ConsejeriaOncologica.idOrdenPago = 0
        }
        ...
        await ConsejeriaOncologica.listaDetalleDespacho(ConsejeriaOncologica.idOrden)

        $('#txtFechaUltimaMenstruacionOnco').datepicker({ // se agrego txtProximaConsulta RQ0002
            todayHighlight: true,
            autoclose: true,
            orientation: "bottom"
        ...
        $('#txtFechaUltimaMenstruacionOnco').mask("Dd/Mm/abcd")
    ...
    Eventos() {
        $("#cboClasisifcacion").on('change', function () {
            ...
            ConsejeriaOncologica.Bloqueo(valorClas)
        });

        $('#btnAgregaTipoConsejeria').on('click', async function () {
            await ConsejeriaOncologica.AgregarTipoConsejeria();
    ...
    async ListarCptConsejeriaOncologica() {


        let formData = new FormData()
        return HttpClient.Post('/Atencion/ListarCptConsejeriaOncologica?area=ConsultaExterna', formData)
            .then(res => {
                if (res.lsClasiPac.table.length > 0) {
                    //$('#cboClasisifcacionAnest').empty();
                    ...
                    alerta(2, 'Error ListarCptConsejeriaOncologica!')
                }
            })
    },
    ...
    async AgregarTipoConsejeria() {
        ...
        if (ConsejeriaOncologica.ExisteProd(objselec)) {
    ...
    async CargarFormData(formData) {
        ...
        formData.append("MotivoConsejeriaOnco", $("#cboMotivoConsejeriaOnco").val());
        formData.append("TemaConsejeriaOnco", $("#cboTemaConsejeriaOnco").val());
        formData.append("AntecedenteFamiliarCoOnco", $("#cboAntecedenteFamiliarCoOnco").val());
        formData.append("AntecedenteFamiliarDescripcionCoOnco", $("#txtAntecedenteFamiliarDescripcionCoOnco").val());
        formData.append("AndriaOnco", $("#txtAndriaOnco").val());
        formData.append("UsoAnticonceptivoOnco", $("#cboUsoAnticonceptivoOnco").val());
        formData.append("EdadPrimeraMenstruacionOnco", $("#txtEdadPrimeraMenstruacionOnco").val());
        formData.append("GestacionOnco", $("#cboGestacionOnco").val());
        formData.append("Perdidas", $("#cboPerdidas").val());
        formData.append("ParidadOnco", $("#txtParidadOnco").val());
        formData.append("EdadPrimerEmbarazoOnco", $("#txtEdadPrimerEmbarazoOnco").val());
        formData.append("EdadPrimeraRelacionSexual", $("#txtEdadPrimeraRelacionSexual").val());
        formData.append("FechaUltimaMenstruacionOnco", $("#txtFechaUltimaMenstruacionOnco").val());
        formData.append("TerapiaReemplazoRenalOnco", $("#cboTerapiaReemplazoRenalOnco").val());
        formData.append("RecomendacionesSugerenciasOnco", $("#txtRecomendacionesSugerenciasOnco").val());

        /////// Enviar datos booleanos
        formData.append("ConsumoTabacoOnco", $('#chkConsumoTabacoOnco').is(':checked') ? 1 : 0);
        formData.append("ConsumoAlcoholOnco", $('#chkConsumoAlcoholOnco').is(':checked') ? 1 : 0);
        formData.append("ObesidadOnco", $('#chkObesidadOnco').is(':checked') ? 1 : 0);
        formData.append("SedentarismoOnco", $('#chkSedentarismoOnco').is(':checked') ? 1 : 0);
        formData.append("ComportamientoSexualInadecuado", $('#chkComportamientoSexualInadecuado').is(':checked') ? 1 : 0);

        formData.append("TratamientoParaFertilidad", $('#chkTratamientoParaFertilidad').is(':checked') ? 1 : 0);
        formData.append("LactanciaMaterna", $('#chkLactanciaMaterna').is(':checked') ? 1 : 0);

        formData.append("lstDiagnosticosConsejeria", JSON.stringify(oTable_DiagnosticosUltimoControlCita.api(true).data().toArray()));
        formData.append("esConsejeria", 2);
    ...
    CargarDetalleConsumo: async function () {
        ...
        for (let i = 0; i < tiposConsejeria.length; i++) {
            ...
            let items = await ConsejeriaOncologica.FactCatalogoServiciosXidTipoFinanciamiento(obj.idProducto, Variables.IdTipoFinanciamiento);
    ...
    InsertFactCatalogo: async function () {
        ...
        if (tiposConsejeria.length > 0) {
            ...
            let detalleConsumo = await ConsejeriaOncologica.CargarDetalleConsumo();

            var formData = new FormData();

            formData.append('IdOrden', ConsejeriaOncologica.idOrden);
            formData.append('idOrdenPago', ConsejeriaOncologica.idOrdenPago);
    ...
    async Iniciar() {
        ConsejeriaOncologica.CargaInicial();
        ConsejeriaOncologica.Eventos();

        ConsejeriaOncologica.InitDatablesDiagnosticosUltimoControlCita()
        ConsejeriaOncologica.InitDatablesTiposConsejeria()

        await ConsejeriaOncologica.TiposClasificacionPaciente();
        await ConsejeriaOncologica.ListarCptConsejeriaOncologica();
        await ConsejeriaOncologica.ListarDxUltimaAtencion();
