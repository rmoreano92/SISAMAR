...
var ConsumoServicio = {
    ...
    plugins() {
        ...
        $('#txtFechaIniLista,#txtFechaFinLista,#txtFechaRealiza, #txtFechaEjecutaProcedimiento').datepicker({
            todayHighlight: true,
            autoclose: true,
            orientation: "bottom"
        ...
        $("#txtHoraRegistraProcedimiento").mask("Hn:Nn");
    },
    ...
    eventos() {
        ...
        $('#btnguardarConsumo').on('click', function () {
            ...
            $.ajax({
                type: 'POST',
                url: "/ConsumoServicio/InsertaFactOrdenServicioConDescripcion?area=Facturacion",
                data: formData,
                dataType: "json",
                cache: false,
                contentType: false,
        ...
        $('#btnAgregarCSAtencion').on('click', async function () {
            ...
            //$("#txtCantidadCpt").val(ConsumoServicio.fechaDiaActual());

            var dt = new Date();
            var time = dt.getHours() + ":" + (dt.getMinutes() < 10 ? ("0" + dt.getMinutes()) : dt.getMinutes())

            $("#txtFechaEjecutaProcedimiento").datepicker("setDate", ConsumoServicio.fechaDiaActual());
            $("#txtHoraRegistraProcedimiento").val(time);
        ...
        var parms = {
            ...
            columns: [
                ...
                },
                {
                    ...
                    createdCell: function (td, cellData, rowData, row, col) {
                        $(td).attr('align', 'left')
                        $(td).text(rowData.fechaEjecucion + ' ' + rowData.horaEjecucion)
                    }
                },
                {
                    width: '50%',
        ...
        if (ConsumoServicio.existeProd(objselec)) {
            ...
            if ($('#txtCantidadCpt').val() > 0) {
                var objRow = {
                    ...
                    fechaEjecucion: $('#txtFechaEjecutaProcedimiento').val(),
                    horaEjecucion: $('#txtHoraRegistraProcedimiento').val(),
                    descripcion: $('#txtDescripcionProcedimiento').val()
