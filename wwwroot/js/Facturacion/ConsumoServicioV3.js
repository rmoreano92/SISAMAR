var ConsumoServicio = {
    ...
    eventos() {
        ...
        $('#btnguardarConsumo').off().on('click', async function () {
            ...
            let factOrdenServicio = await HttpClient.Post('/ConsumoServicio/InsertaFactOrdenServicio?area=Facturacion', formData)

            if (factOrdenServicio.estado) {
                alerta2('info', 'Consumo en el servicio', "N° de orden de pago: " + factOrdenServicio.ordenPago + '\n N° de orden: ' + factOrdenServicio.orden)
                if (!window.location.href.includes('Facturacion/EstadoCuenta')) {
                    ConsumoServicio.ListaConsumoServicioByfechas();
                } else {
                    $('#btnActualizarEstadoCuenta').click()
                }
            ...
            //         if (datos.session) {
            //             if (datos.msjReceta == "") {
            //                 alerta('1', "Se  registro correctamente el consumo");
            //                 $('#modalConsumoServicio').modal('hide');
            //                 alerta2('info', 'Consumo en el servicio', "N° de orden de pago: " + datos.ordenPago + '\n N° de orden: ' + datos.orden)

            //                 Cargando(0)

            //                 if(typeof EstadoCuenta == 'undefined')
            //                 {
            //                     ConsumoServicio.ListaConsumoServicioByfechas();
            //                 } else {
        ...
        $('#btnGuardarInformeProcedimiento').off().on('click', async function () {
        ...
        $('#btnEliminarPS').off().on('click', function () {
        ...
        $('#btnAgregarPS').off().on('click', async function () {
        ...
        $("#modalConsumoServicio").off().on('hidden.bs.modal', async function () {
        ...
        $('#btnEliminarCSAtencion').off().on('click', function () {
            //var objrow = oTable_atencionesEmer.api(true).row('.selected').data();
        ...
        $('#btnActualizaCSAtencion').off().on('click', async function () {
            //var objrow = oTable_atencionesEmer.api(true).row('.selected').data();
            //AdmisionEmergencia.BuscaAtencionesCptCEparaFormatoHIS(objrow.idCuentaAtencion)
            await ConsumoServicio.BuscaAtencionesCptCEparaFormatoHIS(Variables.IdCuentaAtencion)
        });

        $('#btnAgregarCSAtencion').off().on('click', async function () {
            ...
            if (window.location.href.includes('EstadoCuenta')) {
                $('input[name="rbdTipoVenta"]').prop('disabled', true)
                $('#txtFechaRealiza').prop('disabled', true)
            }
        ...
        ////////////////////////////////EVENTOS IMPRIMIR TICKET/////////////////////////////////////////
        $('#tblCSAtencion tbody').on('click', '.ImprimirTicketConsumoServicioSF', async function () {
            let objrow = oTable_consumoServAtencion.api(true).row($(this).parents("tr")[0]).index();
            let row = oTable_consumoServAtencion.fnGetData(objrow);
            //console.log(row);
            Cargando(1);
            if (isEmpty(row)) {
                alerta(2, 'Seleccione un registro por favor.');
            } else {
                let firma = await Utilitario.SeleccionarFirmaDigitalV2(row.codeTicket)
                AbrirVisorDocumento(firma.rutaArchivo, 0);
            }
            Cargando(0);
        });

        $('#tblCSAtencion tbody').on('click', '.ImprimirTicketConsumoServicioCF', async function () {
            let objrow = oTable_consumoServAtencion.api(true).row($(this).parents("tr")[0]).index();
            let row = oTable_consumoServAtencion.fnGetData(objrow);
            //console.log(row);
            Cargando(1);
            if (isEmpty(row)) {
                ...
                await Utilitario.AbrirDocumentoFirmadoBit4Id(row.codeTicket);
            }
            Cargando(0);
        });

        $('#tblCSAtencion tbody').on('click', '.FirmarTicketConsumoServicioSF', async function () {
            let objrow = oTable_consumoServAtencion.api(true).row($(this).parents("tr")[0]).index();
            let row = oTable_consumoServAtencion.fnGetData(objrow);
            ...
            if (isEmpty(row)) {
                ...
                Utilitario.TipoArchivoFirmar = 'TCK-CS';
                Cargando(1);
                let firma = await Utilitario.ValidarUsuarioFirmaDigital(row.codeTicket)               //KHOYOSI            
                if (firma) {
                    if (permisoFirmaDigital == 1) { await Utilitario.IniciarServicioFirmaBit4Id(row.codeTicket); }
                    if (permisoFirmaDigital == 2) { await Utilitario.IniciarServicioFirmaPeru(row.codeTicket); }
            ...
            columns: [
                    ...
                    createdCell: function (td, cellData, rowData, row, col) {
                        ...
                        if (rowData.code != '0') {
                            if (rowData.statusFirma == 1) {
                                btnImprimeSinF = "";
                                btnImprime = ' <button class="btn btn-sm btn-success glow_button ImprimirTicketConsumoServicioCF" title="Imprime Atención Firmada" data-toggle="tooltip" style="margin: 2px;"><i class="fa fa-print"></i> </button>';
                            } else {
                                btnImprimeSinF = '<button class="ImprimirTicketConsumoServicioSF btn btn-sm btn-warning glow_button" title="Visualiza Atencion" data-toggle="tooltip" style="margin: 2px;"><i class="fa fa-eye"></i> </button>';
                                btnRuta = '<button class="btn btn-sm btn-info btnFirmaCE FirmarTicketConsumoServicioSF" title="Firmar Atención" data-toggle="tooltip" style="margin: 2px;"> <i class="fa fa-pencil"></i></button>';
                            }
                        }
                ...
                },
                {
                    ...
                    createdCell: function (td, cellData, rowData, row, col) {
                        ...
                        let botonInforme = `<button title="Agregar Informe Procedimiento" data-toggle="tooltip" type="button" class="btn btn-sm btn-info btnAbrirModalInformeProcedimiento">
                                                <i class="fa fa-pen" style="text-align:center;"></i>
                                            </button>`

                        $(td).html(botonInforme)
        ...
        if (ConsumoServicio.existeProd(objselec)) {
            ...
            if ($('#txtCantidadCpt').val() > 0) {
                var objRow = {
                    ...
                    condicionIngresoDescripcion: $('#cboCondicionIngreso option:selected').text()
        ...
        try {
            ...
            if (datos.table.length > 0) {
                ...
                if (sigue) {
                    ...
                    await ConsumoServicio.ListarProcedimientosRealizadosPorHistoriaTamizaje(datos.table[0].idPaciente)
    ...
    async ConsumoServicioPorCodigoGuardar(idCuentaAtencion, codigoCPT, cantidad) {
        let formData = new FormData();
        let mensaje = "";
        let resp = false;

        formData.append('idCuentaAtencion', idCuentaAtencion);
        formData.append('codigoCPT', codigoCPT);
        ...
        try {
            ...
            datos = await
                $.ajax({
                    method: "POST",
                    url: "/ConsumoServicio/ConsumoServicioPorCodigoGuardar?area=Facturacion",
                    //contentType: "application/json; charset=utf-8",
                    data: formData,
                    dataType: "json",
                    cache: false,
