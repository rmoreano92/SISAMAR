let NotaIngresoRegistrar = {

    eval: false,

    CargaInicial: () => {
        NotaIngresoRegistrar.ListaTiposEmb()
    },
            ...
            success: function (datos) {
                ...
                if (datos.session) {
                    ...
                    if (datos.lstAtencionEvalEmergTotal.table.length > 0) {
                        ...
                        //alert(2);
                        NotaIngresoRegistrar.LlenarRadioDefecto()
                        NotaIngresoRegistrar.ListaEvalEmergencia(objrow.idAtencionEmeg_CE)
                        NotaIngresoRegistrar.ListaGinecoObst(objrow.idAtencionEmeg_CE)
    ...
    CargarDatosEvaluacionHospi: function(idAtencion, idServicioEgreso, idNumero) {
        NotaIngresoRegistrar.ListaEvalEmergenciaByServicioByNumero(idAtencion, idServicioEgreso, idNumero)
        NotaIngresoRegistrar.ListaGinecoObstByServicioByNumero(idAtencion, idServicioEgreso, idNumero)
    },
    ...
    GuardarEvaluacionNeo: async function() {
        ...
        if (eval == 0 && !this.eval) {
            ...
            const data4 = await NotaIngresoRegistrar.GuardarEvaluacionNeoDetalle()
    ...
    IniciaFormulario: async function (accion) {
        ...
        var datosAtencion = "Atención=> Servicio Ingreso:" + objrow.servicioIngreso + " / Fecha Ingreso: " + objrow.fechaIngreso + " / Hora Ing.: " + objrow.horaIngreso + " / Servicio Actual: " + objrow.servicioActual

        $('#hdNroEvaluacion').val(1)
        //idPacienteGlobal = objrow.idPaciente;

        NotaIngresoRegistrar.LimpiarRecetas()
        NotaIngresoRegistrar.LimpiarModal()

        NotaIngresoRegistrar.CargaDatosPorDefecto()

        await Triaje.listaTriajeEmgHosp(objrow.idAtencion, objrow.idServicioEgreso, 0);
        await NotaIngresoRegistrar.ListaAtencionesDatosAdicionalesSeleccionarPorIdCuenta(objrow.idCuentaAtencion)
        await NotaIngresoRegistrar.ListaEvaluaciones()

        await NotaIngresoRegistrar.CargarDatosEvaluacionHospi(objrow.idAtencion, objrow.idServicioEgreso, 1)
        // Cargar Detalle evaluacion
        
        
        // Cargar Detalle evaluacion
        
        /////await NotaIngresoRegistrar.ListaRecetas(objrow.idCuentaAtencion, objrow.idTipoFinanciamiento, 1, 0, 0) // JDELGADO010
    ...
    async IniciarModulo() {
        await NotaIngresoRegistrar.IniciarScript();
    },

    async IniciarScript() {
        //EvaluacionNeonatal.initDatables();
        NotaIngresoRegistrar.CargaInicial()
        NotaIngresoRegistrar.InitDatablesEvaluaciones()
        NotaIngresoRegistrar.Events()
    }
}