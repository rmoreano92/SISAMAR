let EstablecimientosMinsa = {
        ...
        $.ajax({
            ...
            success: function (datos) {
                ...
                if (datos.session) {
                    ...
                    EstablecimientosMinsa.ListaProvincias();
                }
    ...
    TiposEstablecimientos: async function () {
        Cargando(1);

        let formData = new FormData();

        const res = await HttpClient.Post('/Establecimientos/TiposEstablecimientos?area=Comun', formData);

        $('#cboTipoEstablecimiento').empty();
        ...
        $(res.data.table).each(function (i, obj) {
            ...
            $('#cboTipoEstablecimiento').append(`<option value="${obj.idTipo}">${obj.descripcion}</option>`)
        })
    ...
    BuscarEstablecimiento() {
        ...
        if (codigo != "")
            filtro = filtro + " AND Establecimientos.Codigo LIKE '%" + codigo + "%'"
        if (nombre != "")
    ...
    CrearModificarEstablecimientos: async function () {

        let formData = new FormData()

        formData.append('IdEstablecimiento', EstablecimientosMinsa.IdEstablecimiento);
        ...
        formData.append('IdTipo', $('#cboTipoEstablecimiento').val());
        formData.append('Activo', $('#chkActivo').is(':checked') ? 1 : 0);
        formData.append('IdListBarItem', ObtenerItemListBar())

        let res = await HttpClient.Post('/Establecimientos/CrearModificarEstablecimientos?area=Farmacia', formData)
    ...
    SeleccionarEstablecimientoByIdEstablecimiento: async function () {

        let formData = new FormData()

        formData.append('IdEstablecimiento', EstablecimientosMinsa.IdEstablecimiento);

        let res = await HttpClient.Post('/Establecimientos/SeleccionarEstablecimientoByIdEstablecimiento?area=Farmacia', formData)
    ...
    CargarFormulario: async function () {
        ...
        let data = await this.SeleccionarEstablecimientoByIdEstablecimiento()
        ...
        await EstablecimientosMinsa.ListaProvinciasRegistro();

        $('#cboProvincia').val(data.idProvincia)

        await EstablecimientosMinsa.ListaDistritoRegistro();

        $('#cboDistrito').val(data.idDistrito)
        $('#cboTipoEstablecimiento').val(data.idTipo)
    ...
    LimpiarCampos: function () {
        ...
        EstablecimientosMinsa.IdEstablecimiento = 0
    ...
    Events: function () {
        ...
        $('#btnBuscarEstablecimiento').on('click', function () {
            EstablecimientosMinsa.BuscarEstablecimiento();
        });
        ...
        $('#btnAgregar').on('click', function () {
            ...
            EstablecimientosMinsa.LimpiarCampos()
        ...
        $('#btnGuardar').on('click', async function () {
            ...
            if (isEmpty($('#cboTipoEstablecimiento').val())) {
                alerta(2, 'El Tipo del Establecimiento es obligatorio.')
                return
            }

            Cargando(1)
            await EstablecimientosMinsa.CrearModificarEstablecimientos()
        ...
        $('#btnModificar').on('click', async function () {
            ...
            EstablecimientosMinsa.IdEstablecimiento = objRow.idEstablecimiento

            await EstablecimientosMinsa.CargarFormulario()
            Cargando(0)
        ...
        $('#cmbprovEstBuscar').on('change', function () {
            $('#cmbdistEstBuscar').empty();
            EstablecimientosMinsa.ListaDistrito();
        });

        $('#cboDepartamento').on('change', function () {
            $('#cboProvincia').empty();
            $('#cboDistrito').empty();
            EstablecimientosMinsa.ListaProvinciasRegistro();
        });
        $('#cboProvincia').on('change', function () {
            $('#cboDistrito').empty();
            EstablecimientosMinsa.ListaDistritoRegistro();
        });
